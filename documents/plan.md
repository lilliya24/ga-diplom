...
_Отправка формы с пустыми полями;_

+ действия: выбрать способ оплаты по карте, оставить все поля пустыми;
+ ожидаемый результат: cообщение об ошибке под всеми полями “Неверный формат” и "Поле обязательно для заполнения"
+ действия: выбрать способ оплаты через оформление кредита, оставить все поля пустыми;
+ ожидаемый результат: cообщение об ошибке под всеми полями “Неверный формат” и "Поле обязательно для заполнения"



_Отправка формы с невалидными данными в поле “Номер карты" ;_

+ действия: заполнить поле ввода номера карты  невалидными данными, остальные поля заполнить валидными данными;
...
+ действия: заполнить поле ввода "месяц"  невалидными данными (ввести номер месяца больше 12), остальные поля заполнить валидными данными;
...
+ действия: заполнить поле "год" невалидными данными (ввести истекший срок действия карты), остальные поля заполнить валидными данными;
+ ожидаемый результат: cообщение об ошибке "Истёк срок действия карты" под строкой "Год"



_Отправка формы с невалидными данными в поле "Владелец";_
...
_Отправка формы с невалидными данными в поле СVC;_
