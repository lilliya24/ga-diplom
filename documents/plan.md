...
1. _Оплата дебетовой картой APPROVED;_
+ Действия: заполнить все поля валидными данными: 
    - Поле "Номер карты" -  4444 4444 4444 4441
    - Поле "Месяц" - 01
    - Поле "Год" последние две цифры данного года - 24
    - Поле "Владелец" - Иванов Иван 
    - Поле "CVC/CVV" - 123 
...
2. _Оплата дебетовой картой DECLINED;_
+ Действия: заполнить все поля валидными данными:
...
3. _Выдача кредита по данным банковской карты APPROVED;_
+ Действия: заполнить все поля алидными даными:
    - Поле "Номер карты" -  4444 4444 4444 4441
    - Поле "Месяц" - 01
    - Поле "Год" последние две цифры данного года - 24
    - Поле "Владелец" - Иванов Иван
...
4. _Выдача кредита по данным банковской карты DECLINED;_
+ Действия: заполнить все поля валидными данными:
    - Поле "Номер карты" -  4444 4444 4444 4442
    - Поле "Месяц" - 01
    - Поле "Год" последние две цифры данного года - 24
...
7. _Отправка формы с невалидными данными в поле “Номер карты" (ввести меньше 16 символов (например 15));_
+ Действия: заполнить поле ввода номера карты  невалидными данными:
    - Поле "Номер карты" -  4444 4444 4444 444,
...
8. _Отправка формы с невалидными данными в поле "Номер карты" (например, ввести 0000 0000 0000 0000);_
+ Действия: заполнить поле ввода номера карты  невалидными данными:
...
9. _Отправка формы с невалидными данными, поле ввода "Номера карты" оставить пустым._
+ Действия: заполнить поле ввода номера карты  невалидными данными:
    - Поле ввода "Номера карты"  оставить пустым,
+ Остальные поля заполнить валидными данными:
    -  Поле "Месяц" - 01 
    -  Поле "Год" последние две цифры данного года - 24
    - Поле "Владелец" - Иванов Иван
...
10. _Отправка формы  с невалидными данными в поле "Месяц"(цифра больше 12);_
...
11. _Отправка формы с невалидными данными в поле "Месяц"(два нуля);_
...
12. _Отправка формы с невалидными данными в поле "Год"(прошедшая дата);_
...
13. _Отправка формы  с невалидными данными в поле "Год"(больше 5 лет);_
...
14. _Отправка формы  с невалидными данными поле "Год" оставить пустым;_
...
15. _Отправка формы с невалидным значением в поле "Владелец" (ввести цифры 12345);_
...
16. _Отправка формы с невалидным значением в поле "Владелец" (ввести спец.символы ?,%);_
...
17. _Отправка формы с невалидным значением в поле "Владелец" (дефис в начале и конце -Иванов; Иванов-);_
...
18. _Отправка формы  с невалидными данными в поле "Владелец" (Один символ в имени и фамилии И И);_"
...
19. _Отправка формы  с невалидными данными в поле "Владелец" (Поле "Владелец" оставить пустым);_
...
20. _Отправка формы с невалидными данными в поле СVC (ввести меньше 3 цифр) ;_
...
21. _Отправка формы с невалидными данными в поле СVC (ввести 000);_
